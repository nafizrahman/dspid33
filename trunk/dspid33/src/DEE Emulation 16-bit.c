...
{
    unsigned int savedTBLPAG; // Context save of TBLPAG value. Current 
    						  // and packed page are on same page.
    unsigned int currentPage;
    unsigned int pmOffset;   // Current array (page) offset of selected 
    						 // element (PM 16-bit word)
    unsigned int latch;
    unsigned int i;
    unsigned char bank;
